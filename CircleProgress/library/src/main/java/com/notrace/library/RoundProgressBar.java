...
public class RoundProgressBar extends View {
	...
	public RoundProgressBar(Context context, AttributeSet attrs,
		...
		default_suffix_padding = Utils.dp2px(getResources(), 6);
		default_suffix_text = "%";
		default_bottom_text_size = Utils.sp2px(getResources(), 10);
		default_stroke_width = Utils.dp2px(getResources(), 6);// 进度条的宽度

		TypedArray attributes = context.getTheme().obtainStyledAttributes(
				attrs, R.styleable.CircleProgress, defStyleAttr, 0);
	...
	protected void initByAttributes(TypedArray attributes) {
		finishedStrokeColor = attributes.getColor(
				R.styleable.CircleProgress_cir_finished_color,
				default_finished_color);
		unfinishedStrokeColor = attributes.getColor(
				R.styleable.CircleProgress_cir_unfinished_color,
				default_unfinished_color);
		textColor = attributes.getColor(R.styleable.CircleProgress_cir_text_color,
				default_text_color);
		textSize = attributes.getDimension(
				R.styleable.CircleProgress_cir_text_size, default_text_size);
		arcAngle = attributes.getFloat(R.styleable.CircleProgress_cir_angle,
				default_arc_angle);
		...
		strokeWidth = attributes.getDimension(
				R.styleable.CircleProgress_cir_stroke_width, default_stroke_width);
		suffixTextSize = attributes.getDimension(
				R.styleable.CircleProgress_cir_suffix_text_size,
				default_suffix_text_size);
		suffixText = TextUtils.isEmpty(attributes
				.getString(R.styleable.CircleProgress_cir_suffix_text)) ? default_suffix_text
				: attributes.getString(R.styleable.CircleProgress_cir_suffix_text);
		suffixTextPadding = attributes.getDimension(
				R.styleable.CircleProgress_cir_suffix_text_padding,
				default_suffix_padding);
		bottomTextSize = attributes.getDimension(
				R.styleable.CircleProgress_cir_bottom_text_size,
				default_bottom_text_size);
		bottomText = attributes
				.getString(R.styleable.CircleProgress_cir_bottom_text);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		rectF.set(strokeWidth / 2f+5, strokeWidth / 2f+5, width - strokeWidth / 2f-5,
				MeasureSpec.getSize(heightMeasureSpec) - strokeWidth / 2f-5);
		float radius = width / 2f-strokeWidth;
