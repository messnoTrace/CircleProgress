...
public class RoundProgressBar extends View{
	    ...
	    protected void initByAttributes(TypedArray attributes) {
	        finishedStrokeColor = attributes.getColor(R.styleable.CircleProgress_cir_finished_color, default_finished_color);
	        unfinishedStrokeColor = attributes.getColor(R.styleable.CircleProgress_cir_unfinished_color, default_unfinished_color);
	        textColor = attributes.getColor(R.styleable.CircleProgress_cir_text_color, default_text_color);
	        textSize = attributes.getDimension(R.styleable.CircleProgress_cir_text_size, default_text_size);
	        arcAngle = attributes.getFloat(R.styleable.CircleProgress_cir_angle, default_arc_angle);
	        setMax(attributes.getInt(R.styleable.CircleProgress_cir_max, default_max));
	        setProgress(attributes.getInt(R.styleable.CircleProgress_cir_progress, 0));
	        strokeWidth = attributes.getDimension(R.styleable.CircleProgress_cir_stroke_width, default_stroke_width);
	        suffixTextSize = attributes.getDimension(R.styleable.CircleProgress_cir_suffix_text_size, default_suffix_text_size);
	        suffixText = TextUtils.isEmpty(attributes.getString(R.styleable.CircleProgress_cir_suffix_text)) ? default_suffix_text : attributes.getString(R.styleable.CircleProgress_cir_suffix_text);
	        suffixTextPadding = attributes.getDimension(R.styleable.CircleProgress_cir_suffix_text_padding, default_suffix_padding);
	        bottomTextSize = attributes.getDimension(R.styleable.CircleProgress_cir_bottom_text_size, default_bottom_text_size);
	        bottomText = attributes.getString(R.styleable.CircleProgress_cir_bottom_text);
	    }
	    ...
	    protected void onDraw(Canvas canvas) {
	        ...
	        canvas.drawArc(rectF, finishedStartAngle, finishedSweepAngle, false, paint);
	         ...
	         drawRedDot(canvas, finishedSweepAngle, getWidth()/2, getHeight()/2, getWidth()/2-10);
	        ...
	        if (!TextUtils.isEmpty(text)) {
	            ...
	            canvas.drawText(suffixText, getWidth() / 2.0f  + textPaint.measureText(text) + suffixTextPadding, textBaseline , textPaint);
	        }
    ...
    protected void drawRedDot(
        ...
        canvas.drawCircle(xCenter + (float) (radius * Math.cos(dotRadians)),
                yCenter + (float) (radius * Math.sin(dotRadians)), 5, paint);
    }
